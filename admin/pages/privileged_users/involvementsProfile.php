<?php
    ...
    $inv_encode = mysqli_real_escape_string($obj->CONN, trim($_GET['inv']));
    $gettedinv = base64_decode(urldecode($inv_encode));

    $validateINV = "SELECT * FROM permissions WHERE email = '".$gettedinv."' AND disflag = '0' ";
    $data = $obj->select($validateINV);
    if (count($data) == 0){
        header("Location:../../"); 
        die;
    }
    ...
    if (isset($_POST['update_profile'])){
        if((isset($_POST['email'])) and (isset($_POST['name'])) and (isset($_POST['phone']))){
          ...
          if((!empty($_POST['email'])) and (!empty($_POST['name'])) and (!empty($_POST['phone']))){
              ...
              if ((count($emaildata) == 0) or ((count($emaildata) > 0) and ($emaildata[0]['email'] == $gettedinv))){
                    $updatepldrsql = "UPDATE assigned_pleaders SET name = '".$name."', email = '".$email."', phone = '".$phone."' WHERE email = '".$gettedinv."' ";
                    $obj->edit($updatepldrsql);
                    
                    $updatepersql = "UPDATE permissions SET name = '".$name."', email = '".$email."', phone = '".$phone."' WHERE email = '".$gettedinv."' ";
                    $obj->edit($updatepersql);

                    echo "<script>alert('Profile Updated Successfully.');</script>";
                    header("Location:../privileged_users/involvementsProfile.php?inv=".urlencode(base64_encode($email)).""); die;
              }else{
...
<html lang="en">
<head>
  <title>Admin | Associated's Profile</title>
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0"><i class="nav-icon fas fa-user"></i> &nbsp;&nbsp;Associated's Profile</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="../dashboard">Home</a></li>
              <li class="breadcrumb-item active font-weight-bold">Associated's Profile</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
        ...
        <div class="row">
          <div class="col-12">
            ...
            <div class="card">
                ...
                (<strong>Note:</strong> By Updating Associated's Profile Details, changes will be reflected to the entire system.)
                <a class='btn btn-sm btn-flat btn-danger font-weight-bold float-right' href = '../privileged_users'><i class='nav-icon fas fa-arrow-left'></i> &nbsp; Back</a>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="card-body table-responsive p-0" style="height: 300px;">
                        <table class="table table-hover table-bordered table-head-fixed text-nowrap">
                            ...
                            <tbody>
<?php
    $sql = "SELECT case_id, role FROM permissions WHERE email = '".$gettedinv."' AND disflag = '0' ";
...
?>
    ...
    ?>
                                    ...
                                    <td><?php echo $querydata[$i]['role']; ?></td>
...
$(document).on("submit","#profileform",function(e) {
    if (confirm("Are You Sure to change the profile information?") == true) {
        return true;
    } else {
        return false;
    }
